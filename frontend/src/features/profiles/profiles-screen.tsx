...
type ProfileID = 'recommended' | 'maximum'
...
export default function ProfilesScreen() {
  ...
  const [profile, setProfile] = useState<ProfileID>('recommended')
  const load = useCallback(async (signal: AbortSignal): Promise<TweakData> => {
    const [recommended, maximum, checkpoint] = await Promise.all([
      client.call<Plan>('optimization.plan', { profile: 'recommended' }, signal),
      client.call<Plan>('optimization.plan', { profile: 'maximum' }, signal),
      client.call<CheckpointStatus>('optimization.checkpoint_status', { profile }, signal),
    ])
    return { recommended, maximum, checkpoint }
  }, [client, profile])
  ...
  return (
    <div className="page">
      ...
      {section === 'tweaks' && resource.data ? <TweakCatalog plan={resource.data.maximum} onChanged={(message) => { setResult(message); resource.refresh() }} /> : null}
      {section === 'profiles' && resource.data && selectedPlan ? (
        <>
          <div className="preset-grid">
            <PresetCard id="recommended" selected={profile === 'recommended'} title={c.recommended} subtitle={c.recommendedSubtitle} description={c.recommendedDescription} count={resource.data.recommended.items.length} risk={c.balancedRisk} icon={<ShieldCheck size={22} />} onSelect={setProfile} />
            <PresetCard id="maximum" selected={profile === 'maximum'} title={c.maximum} subtitle={c.maximumSubtitle} description={c.maximumDescription} count={resource.data.maximum.items.length} risk={c.maximumRisk} icon={<Zap size={22} />} onSelect={setProfile} />
          </div>
          ...
          <section className="preset-summary panel panel--gold"><div><span>{c.selectedPreset}</span><h2>{profile === 'maximum' ? c.maximum : c.recommended}</h2><p>{profile === 'maximum' ? c.maximumIntent : c.recommendedIntent}</p></div><div className="preset-summary__counts"><strong>{selectedPlan.items.length}</strong><span>{c.tweaksCount}</span><strong>{changed}</strong><span>{c.toChange}</span></div></section>
...
function localizePlan(value: string, language: 'ru' | 'en'): string {
  ...
  const map: Record<string, string> = {
    'Игры': 'Gaming', 'Ввод': 'Input', 'Интерфейс': 'Interface', 'Питание': 'Power',
