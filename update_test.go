...
func TestUpdateConfigIsReplaceableAndRecoverable(t *testing.T) {
	dir := t.TempDir()
	oldConfigDir := userConfigDir
	userConfigDir = func() (string, error) { return dir, nil }
	defer func() { userConfigDir = oldConfigDir }()
	if err := saveUpdateConfig(updateConfig{Auto: true}); err != nil {
		t.Fatal(err)
	}
	if err := saveUpdateConfig(updateConfig{Auto: false}); err != nil {
		t.Fatal(err)
	}
	...
	if err := updateCommand([]string{"disable"}); err != nil {
		t.Fatal(err)
	}
	config, err := loadUpdateConfig()
	if err != nil || config.Auto {
