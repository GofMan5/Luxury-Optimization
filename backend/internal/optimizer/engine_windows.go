	...
	if profile.NetworkLatency {
		...
		if err != nil {
			...
			for _, property := range properties {
				plan.Items = append(plan.Items, PlanItem{ID: networkTweakID(property), Category: "Ethernet", Name: property.AdapterName + ": " + property.Keyword, Current: strings.Join(property.Values, ","), Desired: "0 (отключено)", Changed: networkPropertyNeedsChange(property.Values)})
			}
		}
	}
	return plan, nil
...
func createCheckpoint(profileID, id string) (string, error) {
	if !isAdministrator() {
		return "", errors.New("для создания точки восстановления нужны права администратора")
	}
	if !backupIDPattern.MatchString(id) {
		return "", errors.New("неверный ID точки восстановления")
	}
	releaseLock, err := acquireOperationLock()
	if err != nil {
		return "", err
	...
	if registryUserSID == "" {
		return "", errors.New("не задан пользователь точки восстановления")
	}
	stateDir, err := ensureStateDir()
	if err != nil {
		return "", err
	}
	backup := Backup{FormatVersion: 1, CatalogVersion: 1, ID: id, CreatedAt: time.Now().UTC(), Profile: profile.ID, TargetUserSID: registryUserSID, Status: "checkpoint"}
	backup.Path = filepath.Join(stateDir, backup.ID+".json")
	for _, change := range profile.Changes {
		snapshot, err := snapshotRegistry(change)
		if err != nil {
			return "", fmt.Errorf("точка восстановления %s: %w", change.ID, err)
		}
		backup.Registry = append(backup.Registry, snapshot)
	}
	backup.AppliedRegistry = len(backup.Registry)
	backup.CatalogDigest = backupTargetDigest(backup.Registry)
	if profileChangesMouse(profile.Changes) {
		backup.Mouse, err = captureMouseParameters()
		if err != nil {
		...
		backup.Mouse.Applied = backup.Mouse.Captured
	}
	if profile.PerformancePlan {
		backup.Power.PreviousGUID, err = activePowerGUID()
		if err != nil {
	...
	if profile.NetworkLatency {
		properties, err := queryNetworkProperties()
		if err != nil {
			return "", fmt.Errorf("точка восстановления Ethernet: %w", err)
		}
		backup.Network = networkChanges(properties)
		backup.NetworkApplied = len(backup.Network) > 0
	}
	if err := saveBackup(&backup); err != nil {
		return "", err
	}
...
func hasRecentCheckpoint(targetSID, profile string, maximumAge time.Duration) (bool, error) {
	_, names, err := backupFiles()
	if err != nil {
		return false, err
	}
	...
	for _, name := range names {
		...
		backup, err := loadBackupFile(filepath.Join(directory, name), id)
		if err != nil {
			return false, err
		}
		age := now.Sub(backup.CreatedAt)
		if backup.Status == "checkpoint" && backup.TargetUserSID == targetSID && backup.Profile == profile && age >= 0 && age <= maximumAge {
			return true, nil
		}
	}
	return false, nil
...
func verifyApplied(profile Profile, backup Backup) error {
	...
	if backup.Mouse.Applied {
		...
		if isMouseTweak(backup.TweakID) {
			if mouseTweakValue(current, backup.TweakID) != 0 {
				return fmt.Errorf("live-параметр %s не применён", backup.TweakID)
			}
		} else if current.Threshold1 != 0 || current.Threshold2 != 0 || current.Speed != 0 {
	...
	if backup.Mouse.Applied && !registryFailed {
		...
		if isMouseTweak(backup.TweakID) {
			...
			if err != nil {
				...
				desired = current
				setMouseTweakValue(&desired, backup.TweakID, mouseTweakValue(backup.Mouse, backup.TweakID))
			}
		}
		if mouseErr == nil {
			mouseErr = applyMouseParameters(desired)
		}
		if mouseErr == nil {
			current, err := captureMouseParameters()
			if err != nil {
				mouseErr = err
			} else if isMouseTweak(backup.TweakID) && mouseTweakValue(current, backup.TweakID) != mouseTweakValue(backup.Mouse, backup.TweakID) {
				mouseErr = errors.New("live-параметр мыши не совпал с backup после отката")
			} else if !isMouseTweak(backup.TweakID) && (current.Threshold1 != backup.Mouse.Threshold1 || current.Threshold2 != backup.Mouse.Threshold2 || current.Speed != backup.Mouse.Speed) {
				mouseErr = errors.New("live-параметры мыши не совпали с backup после отката")
			}
		}
		if mouseErr != nil {
			problems = append(problems, mouseErr)
		} else {
...
func restoreBackup(requestSID, backupID string, boostSession bool) (string, error) {
	...
	if !backupRestorable(backup.Status) {
	...
	defer func() { _ = setRegistryUserSID("") }()
	rollbackErr := rollbackBackup(&backup)
	if boostSession || backup.TweakID != "" {
		return backup.Path, rollbackErr
	}
	return backup.Path, errors.Join(rollbackErr, restoreStartupBackups(), restoreServiceBackups())
}
...
func validateBackup(backup Backup) error {
	...
	if backup.TweakID != "" {
		if err := validateTweakBackupShape(backup, allowed); err != nil {
			return err
		}
	}
