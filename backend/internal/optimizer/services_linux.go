...
func listServices(state, match string) (ServicesReport, error) {
	...
	for _, line := range strings.Split(units, "\n") {
		...
		entry := ServiceEntry{Name: fields[0], State: fields[3], StartType: startTypes[fields[0]], Display: description, Description: description, System: true, Manageable: false}
		if entry.StartType == "" {
