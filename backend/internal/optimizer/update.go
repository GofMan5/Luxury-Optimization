...
func updateCommand(args []string) error {
	...
	case "status":
		...
		fmt.Printf("Auto update: enabled\nRelease channel: %s.x\nLast check: %s\n", releaseChannel, formatTime(config.LastCheck))
		return nil
	case "enable":
		...
		return nil
	case "disable":
		return errors.New("автоматические проверки обновлений всегда включены")
	case "check", "install":
		set := flag.NewFlagSet("update "+action, flag.ContinueOnError)
...
func maybeAutoUpdate() {
	...
	if time.Since(config.LastCheck) < 24*time.Hour {
		return
	}
...
func RunAutoUpdate() { maybeAutoUpdate() }
