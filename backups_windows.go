...
func listBackupSummaries(targetSID string) ([]BackupSummary, error) {
	...
	for _, name := range names {
		...
		restorable := backup.Status == "applied" || backup.Status == "prepared" || backup.Status == "failed" || backup.Status == "rolling_back" || backup.Status == "rollback_failed"
		result = append(result, BackupSummary{ID: backup.ID, CreatedAt: backup.CreatedAt, Profile: backup.Profile, Status: backup.Status, Restorable: restorable})
	}
	return result, nil
}
